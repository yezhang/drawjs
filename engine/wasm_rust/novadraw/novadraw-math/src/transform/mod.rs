...
impl Transform {
    ...
    pub fn transform_point(self, point: Vec2) -> Vec2 {
        self.matrix.transform_point(point) + self.translation
    }
    ...
    pub fn multiply(self, other: Transform) -> Transform {
        ...
        let scaled_trans = self.matrix.transform_vector(other.translation);
    ...
    pub fn inverse(self) -> Option<Transform> {
        self.matrix.inverse().map(|inv_matrix| {
            let inv_translation = -inv_matrix.transform_vector(self.translation);
            Transform {
                matrix: inv_matrix,
