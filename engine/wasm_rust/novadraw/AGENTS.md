...
| 渲染热路径 | 不打印日志 |
| 渲染主循环保护 | `render_recursive.rs` 与 `render_iterative.rs` 主循环通常不改；除非已对标 draw2d 证明主流程不符 |
...
- 禁止全局状态（Singleton）。
- 渲染热路径禁止打印日志。
- `render_recursive.rs` 与 `render_iterative.rs` 是渲染主循环保护区，通常不应改主循环逻辑；除非已对标 draw2d 证明当前主流程不符。
