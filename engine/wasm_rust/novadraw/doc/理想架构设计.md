...
| `NovadrawSystem` | 无 | **新增**：统一组合根；公开 trait 只暴露稳定入口，平台实现内部装配 graph / update manager / dispatcher / scene host | 新增 |
...
8. **平台输入适配**
   - app_window 处理 winit 事件到组合根命名动作的转换
   - EventDispatcher 保持平台无关，只接收逻辑入口域事件点
