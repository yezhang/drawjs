...
- `Viewport`
  ...
  - 约束：不改变 `DamageSet`/`find_mouse_event_target_at()` 的核心语义，只扩展坐标与裁剪规则
- `ScrollPane / Scroll`
...
└─────────────────────────────────────────────────────────────────────────────────────┘
        ...
        └───────────────────┘       └───────────────────┘              │
                                        ...
                                        ▼    │                         ▼
                              ┌───────────────────┐          ┌───────────────────┐
                              │   FigureBlock    │          │WinitEventDispatcher│
                              │   (节点状态)      │          │   (平台实现)       │
                              │                   │          │                   │
                              │ + id, parent      │          │ + Window 持有     │
                              │ + children        │          └───────────────────┘
...
```text
NovadrawSystem (trait)                           ← 全局唯一组合根
  │
  ├── scene: FigureGraph                        // 树结构管理
  │     │
  ...
  │
  ├── update_manager: Arc<dyn UpdateManager>     // 全局更新调度
  │
  ├── dispatcher: Arc<dyn EventDispatcher>      // 全局事件分发
 │     └── 不持有任何引用，通过 DispatchContext 获取命中测试/交互状态/目标分发能力
  │
  └── scene_host: Arc<dyn SceneHost>            // 渲染入口协调
...
│                              平台实现层                                             │
│                                                                                     │
│  WinitNovadrawSystem  ──► 持有 winit::Window                                      │
│  WinitEventDispatcher  ──► 持有 winit::Window                                     │
│  WinitSceneHost       ──► 持有 winit::Window                                      │
...
| 持有 UpdateManager | **NovadrawSystem.update_manager** | 全局唯一服务 |
| 持有 EventDispatcher | **NovadrawSystem.dispatcher** | 全局唯一服务 |
| 持有 Canvas | **WinitEventDispatcher** | 持有 winit Window |
| 注册事件监听 | **WinitEventDispatcher** | addListeners() |
| paint() → UM.paint() | **SceneHost.execute_update()** | 渲染入口 |
| controlResized() | **外部调用** | 设置 viewport + set_bounds() |
| setControl() | **WinitEventDispatcher** | 绑定窗口 |

**关键结论**：

- **SceneHost 不持有任何核心对象**，只负责渲染入口协调
- LightweightSystem 的"持有"职责被**分散**到 FigureGraph、NovadrawSystem、EventDispatcher trait、WinitEventDispatcher
- **UpdateManager 和 EventDispatcher 都是全局唯一服务**，提升到 NovadrawSystem 层级，与 g2 LightweightSystem 直接持有保持一致
...
///
/// - FigureGraph：通过参数传递
/// - UpdateManager：通过参数传递（来自 NovadrawSystem）
/// - EventDispatcher：在 NovadrawSystem 层级
/// - 平台窗口：WinitEventDispatcher 持有
...
```text
winit event
  → NovadrawSystem（平台实现）创建 DispatchContext（借用 graph + update_manager）
...
│  Novadraw 组件分工                                                   │
│                                                                     │
│  NovadrawSystem    → 全局组合根，持有 UpdateManager + EventDispatcher│
│  FigureGraph       → 树结构 + 交互状态                               │
│  SceneHost         → 渲染入口 + 视口大小 + 更新请求                    │
│  EventDispatcher   → 事件分发（trait）                              │
│  WinitEventDispatcher → 平台实现（持有 Window）                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
draw2d 中 **bounds 是绝对坐标**，这是整个坐标系统的基础：

```java
// Figure.java - bounds 是绝对坐标
...
| 概念 | 说明 |
|------|------|
| **绝对坐标** | bounds 的 (x, y) 是相对于**坐标根**的位置 |
...
1. **事件坐标约定**
   - `EventDispatcher` 接收的 `(x, y)` 是场景根坐标系（RootFigure/SceneRoot）的坐标
...
│  交互状态: mouse_target, focus_owner, captured                              │
│                                                                              │
│  注意: FigureGraph 不持有 UpdateManager（全局唯一，在 NovadrawSystem）      │
└─────────────────────────────────────────────────────────────────────────────┘
                                  │
                                  │ Arc<dyn Trait> 注入
                                  ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  全局服务（在 NovadrawSystem 层级）                                        │
│                                                                              │
│  UpdateManager (Trait)         EventDispatcher (Trait)                     │
│      │                              │                                      │
│      ├── SceneUpdateManager        ├── WinitEventDispatcher (winit 平台)   │
...
```
NovadrawSystem (Novadraw)            LightweightSystem (draw2d)
┌─────────────────────────┐        ┌──────────────────────────┐
│  scene: FigureGraph    │◀───────│  IFigure root           │
│                         │        │  (持有 Figure 树)        │
...
2. **UpdateManager 和 EventDispatcher 都是全局唯一服务**，提升到 NovadrawSystem 层级
...
impl WinitEventDispatcher {
    fn dispatch_mouse_pressed(&mut self, ctx: &mut dyn DispatchContext, x: f64, y: f64, button: MouseButton) {
        ...
        let handled = ctx.dispatch_to_target(target_id, &MouseEvent::Pressed { x, y, button });
        if handled {
    ...
    fn dispatch_mouse_moved(&mut self, ctx: &mut dyn DispatchContext, x: f64, y: f64) {
        ...
        if let Some(captured_id) = ctx.captured() {
            ctx.dispatch_to_target(Some(captured_id), &MouseEvent::Moved { x, y });
        } else {
            let target_id = ctx.mouse_target();
            ctx.dispatch_to_target(target_id, &MouseEvent::Moved { x, y });
        }
    }
    ...
    fn dispatch_mouse_released(&mut self, ctx: &mut dyn DispatchContext, x: f64, y: f64, button: MouseButton) {
        self.receive(ctx, x, y);
        if let Some(captured_id) = ctx.captured() {
            ctx.dispatch_to_target(Some(captured_id), &MouseEvent::Released { x, y, button });
            ctx.set_captured(None);  // 释放捕获
        } else {
            let target_id = ctx.mouse_target();
            ctx.dispatch_to_target(target_id, &MouseEvent::Released { x, y, button });
        }
        self.receive(ctx, x, y);
    }
}
...
| **平台实现** | WinitEventDispatcher | ✅ winit 特定 |
...
/// - 平台实现（如 WinitEventDispatcher）负责将平台事件转换为调用
///
/// # 与 g2 的关键区别
///
...
/// # 设计说明
///
/// 交互状态（mouseTarget、focusOwner、captured）是纯逻辑状态，
...
- `EventDispatcher` 不持有 `mouse_target`、`focus_owner`、`captured`
...
#### WinitEventDispatcher 实现示例

```rust
/// WinitEventDispatcher - 无状态事件分发
///
/// 通过 DispatchContext 访问命中测试、交互状态与分发入口。
pub struct WinitEventDispatcher;
...
顶层事件分发开始
  ...
  → dispatch_to_target()
      ...
      → Figure 通过 MutationContext.allocate_block() 预分配 BlockId（不挂树）
      → Figure 通过 MutationContext.enqueue_mutation() 记录结构性变更
顶层事件分发结束
...
- `NovadrawContext`：继续只负责当前节点修改和更新请求
- `DispatchContext`：继续只负责命中测试、交互状态、目标分发
- 若未来支持结构性变更，新增 `MutationContext`，不要继续膨胀 `NovadrawContext`
...
当前 Novadraw 的组件分散在各处：
- FigureGraph 持有树结构和 UpdateManager
- EventDispatcher 是独立 trait
- WinitEventDispatcher 持有 Window
- SceneHost 只负责渲染入口

**问题**：用户需要理解多个组件，使用不够直观。

### 解决方案：NovadrawSystem trait
...
///
/// 用户通过此接口使用 Novadraw，平台特定实现由 `platform::create_system()` 创建
...
pub struct WinitNovadrawSystem {
    scene: FigureGraph,
    update_manager: Arc<dyn UpdateManager>,     // 全局唯一
    dispatcher: Arc<dyn EventDispatcher>,       // 全局唯一
    scene_host: Arc<dyn SceneHost>,
}
...
impl NovadrawSystem for WinitNovadrawSystem {
    ...
    fn render(&mut self, renderer: &mut impl RenderBackend) -> NdCanvas {
        // UpdateManager 和 SceneHost 都是全局服务，通过参数传递
...
pub mod platform {
    ...
    /// 创建 Winit 平台系统
    pub fn create_system(window: winit::window::Window) -> Box<dyn NovadrawSystem> {
        Box::new(WinitNovadrawSystem::new(window))
    }

    /// 创建 Web 平台系统（未来）
    #[cfg(feature = "web")]
    pub fn create_web_system(canvas: web_sys::HtmlCanvas) -> Box<dyn NovadrawSystem> {
        Box::new(WebNovadrawSystem::new(canvas))
    }
}
```
...
│                         用户代码                                        │
│                                                                     │
│   let mut system = platform::create_system(window);                  │
│   system.scene().add_child(...);                                    │
│   system.render(&mut renderer);                                    │
└─────────────────────────────────────────────────────────────────────┘
                              │
                              │ Box<dyn NovadrawSystem>
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
...
│                 │  │                 │  │                 │
│ - FigureGraph   │  │ - FigureGraph   │  │                 │
│ - UpdateManager │  │ - UpdateManager │  │                 │
│ - WinitEventDisp│  │ - WebEventDisp  │  │                 │
...
fn main() {
    ...
    // 构建场景
    system.scene().set_contents(Box::new(root_figure));
    system.scene().add_child(parent_id, Box::new(child));

    // 事件处理（示意）
    //
    // 实际上 EventDispatcher 会在平台事件循环中被调用，并由 NovadrawSystem 提供 DispatchContext。
    // 这里仅表达语义：事件分发使用 receive + dispatch_to_target 模型。
    let mut dispatch_ctx = /* system.make_dispatch_context() */;
    system
        .dispatcher()
        .dispatch_mouse_pressed(&mut dispatch_ctx, x, y, MouseButton::Left);

    // 触发更新（标记需要验证的节点）
    system.update_manager().add_invalid_figure(block_id);
...
| 组件 | 当前实现 | 理想设计 | 变化 |
|---|---|---|---|
| `FigureBlock` | 已有 | 合并 `layout_valid` 到 `is_valid` | 待做 |
...
| `FigureGraph` | 有 `layout_manager` 和 `update_manager` | **移除两者**，layout_manager 只在 FigureBlock，UpdateManager 在 NovadrawSystem | 待做 |
| `LayoutManager` | 已有 trait | 保持，纯接口设计 | 接收 BlockId 参数 |
| `UpdateManager` | 在 FigureGraph 内 | **移到 NovadrawSystem 层级**，与 EventDispatcher 同级 | 待做 |
| `SceneHost` | 只负责渲染入口协调 | **职责明确**：不持有对象，只做协调 | ✅ 设计正确 |
| `RootFigure` | 有 | **保留作为平台桥接层** | 设计明确 |
| `EventDispatcher` | trait 设计 | **通过 DispatchContext 访问 Graph**，find_mouse_event_target_at 在 Graph 上 | ✅ 设计明确 |
| `NovadrawSystem` | 无 | **新增**：统一组合根，trait + 平台实现，包含 update_manager | 新增 |
...
- **UpdateManager 层级确定**：与 EventDispatcher 同级，都在 NovadrawSystem 层级
- **NovadrawContext 模式**：Figure 通过上下文执行操作，与系统解耦
- **事件分发链路**：对齐 g2 的目标分发（无通用冒泡），包含 mouse_target 与 captured 语义

### P0：立即可做

1. **从 FigureGraph 移除 `layout_manager` 字段**
   - LayoutManager 只在 FigureBlock 中（每个容器独立）
   - FigureGraph 不需要全局 layout_manager

2. **从 FigureGraph 移除 `update_manager` 字段**
   - UpdateManager 是全局唯一服务
   - 应该提升到 NovadrawSystem 层级，与 EventDispatcher 同级

3. **合并 `layout_valid` 到 `is_valid`**
   - draw2d 只有 FLAG_VALID，没有单独的 layout_valid
   - 简化状态管理

4. **实现完整的 WinitEventDispatcher**
   - EventDispatcher 不持有任何引用，通过 DispatchContext 访问 Graph
   - find_mouse_event_target_at：ctx.find_mouse_event_target_at()
   - 目标分发入口：ctx.dispatch_to_target()

### P1：短期

1. **在 FigureGraph 中添加交互状态字段**
   - mouse_target、focus_owner、captured
   - 提供 set/get 方法

2. **实现 NovadrawSystem trait 和 WinitNovadrawSystem**
   - 定义 NovadrawSystem trait
   - 实现 WinitNovadrawSystem
   - 提供 platform::create_system() 工厂

3. **完善 FigureBlock.get_preferred_size() 三级回退**
   - 当前只实现了缓存 → bounds
   - 需要补充 LayoutManager 计算层

4. **明确 RootFigure 作为平台桥接层**
   - 定义 RootFigure 应该委托哪些平台资源
   - 与 NdCanvas 的资源管理对应

5. **最小 Layer/Viewport 原型（作为内核回归基准）**
   - Layer：验证绘制顺序与命中测试顺序一致（子节点逆序命中、z-order 稳定）
   - Viewport：验证坐标根 + client_area 裁剪 + damage 裁剪协定可闭环驱动渲染
   - 定稿：`add_dirty_region(block_id, rect)` 的 rect 坐标空间（必须与坐标根/Viewport 语义一致）

### P2：中期

6. **实现完整的 damage repair 算法**
   - 脏区向父链传播并取交集
   - 对应 draw2d DeferredUpdateManager.repairDamage()

7. **实现 find_mouse_event_target_at 逻辑**
   - EventDispatcher 需要通过 find_mouse_event_target_at 找到目标节点
   - 对应 draw2d Figure.findMouseEventTargetAt()

### P3：长期

8. **EventDispatcher 平台实现**
   - WinitEventDispatcher 实现
   - 处理 winit 事件到 Figure 事件的转换

9. **SceneHost 与 FigureGraph 解耦**
   - SceneHost 只持有 `Arc<dyn SceneUpdateTarget>`
   - FigureGraph 实现 SceneUpdateTarget trait

10. **Scroll（ScrollPane/Viewport + content offset）**
...
| `SWTEventDispatcher.java` | `WinitEventDispatcher` | ⚠️ 待实现 |
...
- 具体的自动测试生成规则、验证层级、文件放置建议，见 `agent/quality-testing-strategy.md`
