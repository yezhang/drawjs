...
| C-02 | FigureBlock 是节点运行时状态容器 | partially_aligned | `AD-009` verified; `AD-011` verified; `CAD-007` candidate | FigureBlock 已移除 selection overlay 绘制行为；AD-011 只处理 FigureGraph 存储面，FigureBlock 字段/方法公开面已登记为 CAD-007 待 REVIEW |
...
| C-09 | 新接口优先表达职责边界，不为兼容现状引入职责回流 | partially_aligned | `AD-010` verified; `AD-011` verified; `CAD-004` / `CAD-005` candidates | 公开可变系统逃生口与 FigureGraph 存储逃生口已移除；FigureBlock 公开面、组合根只读面和理想文档旧表述仍可能引入职责回流 |
