...
- 背景：某个已暂停历史主题被记录在 checkpoint、roadmap、产品清单和 demo matrix 的热路径中，导致恢复工作流时被反复带入抽象层级不同的核心主线讨论。
- 决策：热路径不保留已剥离主题的具体名称、恢复条件或 保留或延期 标记；当前 M2-M7 只讨论通用 Figure 树、父链坐标协议、绘制、布局、事件和通知。
- 规则：已剥离主题不得作为当前 next step、阻塞项或残余风险输出；必要历史只保存在 archive / worklog 等冷路径审计记录中。
- 影响：恢复入口只表达通用边界规则，避免具体关键词成为新的上下文锚点。
