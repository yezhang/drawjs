...
description: "Discovers candidate architecture deltas from the gap between ideal architecture and current code. Invoke when starting a new phase, backlog feels stale, or after finishing a delta."
---
...
5. 将发现的问题写成 candidate delta，并标注它更像：
   ...
   - 调度边界错误
   - 文档契约缺口
6. 为每个 candidate 标注 `evolution_kind`：
   - architecture
   - parity
   - new_feature
7. 判断 candidate 是：
...
8. 为建议 promote 的项补齐优先级、证据、完成标准和建议入口文件。
...
- 如果某项属于 `new_feature`，明确指出它不属于当前默认主线，不要伪装成架构收敛或 parity 缺口
