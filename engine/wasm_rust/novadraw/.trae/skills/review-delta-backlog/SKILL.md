...
description: "Reviews and curates the architecture backlog by deduplicating, splitting, and reprioritizing items. Invoke when backlog gets stale, noisy, or after a few deltas are done."
---
...
5. 检查每个条目的 `evolution_kind` 是否正确，避免把目标外新功能混入主线
6. 按“职责边界优先于实现细节”的原则重排优先级
7. 对过大问题给出拆分建议
8. 明确当前最值得执行的一个 delta
...
- 如果条目属于 `new_feature`，默认不要把它推荐为当前主线
