...
fn create_scene_0_mouse_enter_exit() -> novadraw::FigureGraph {
    ...
    let hover_area = novadraw::RectangleFigure::new_with_color(
        300.0,
        200.0,
        200.0,
        150.0,
        novadraw::Color::rgba(0.4, 0.6, 0.8, 1.0),
    );
    let _area = scene.add_child_to(container_id, Box::new(hover_area));

    let info = novadraw::RectangleFigure::new_with_color(
        50.0,
        50.0,
        700.0,
        40.0,
        novadraw::Color::rgba(0.9, 0.9, 0.8, 1.0),
    );
...
fn create_scene_1_mouse_hover() -> novadraw::FigureGraph {
    let mut scene = novadraw::FigureGraph::new();
    let container = novadraw::RectangleFigure::new(0.0, 0.0, WINDOW_WIDTH, WINDOW_HEIGHT);
    let container_id = scene.set_contents(Box::new(container));
    ...
    let hover_area = novadraw::RectangleFigure::new_with_color(
        ...
        300.0,
        200.0,
        novadraw::Color::rgba(0.3, 0.7, 0.5, 1.0),
    );
    let _area = scene.add_child_to(container_id, Box::new(hover_area));

    let status = novadraw::RectangleFigure::new_with_color(
        50.0,
        500.0,
        700.0,
        50.0,
...
fn create_scene_2_mouse_down_up() -> novadraw::FigureGraph {
    let mut scene = novadraw::FigureGraph::new();
    let container = novadraw::RectangleFigure::new(0.0, 0.0, WINDOW_WIDTH, WINDOW_HEIGHT);
    let container_id = scene.set_contents(Box::new(container));

    let click_area = novadraw::RectangleFigure::new_with_color(
        300.0,
        200.0,
        200.0,
        100.0,
        novadraw::Color::rgba(0.7, 0.4, 0.4, 1.0),
    );
...
fn create_scene_3_mouse_move_basic() -> novadraw::FigureGraph {
    let mut scene = novadraw::FigureGraph::new();
    let container = novadraw::RectangleFigure::new(0.0, 0.0, WINDOW_WIDTH, WINDOW_HEIGHT);
    let container_id = scene.set_contents(Box::new(container));

    let draggable = novadraw::RectangleFigure::new_with_color(
        350.0,
        250.0,
        100.0,
        60.0,
        novadraw::Color::rgba(0.5, 0.5, 0.8, 1.0),
    );
...
fn create_scene_4_mouse_drag() -> novadraw::FigureGraph {
    let mut scene = novadraw::FigureGraph::new();
    let container = novadraw::RectangleFigure::new(0.0, 0.0, WINDOW_WIDTH, WINDOW_HEIGHT);
    let container_id = scene.set_contents(Box::new(container));
...
fn create_scene_5_key_down_up() -> novadraw::FigureGraph {
    let mut scene = novadraw::FigureGraph::new();
    let container = novadraw::RectangleFigure::new(0.0, 0.0, WINDOW_WIDTH, WINDOW_HEIGHT);
    let container_id = scene.set_contents(Box::new(container));

    let key_display = novadraw::RectangleFigure::new_with_color(
        250.0,
        200.0,
        300.0,
        150.0,
        novadraw::Color::rgba(0.4, 0.5, 0.6, 1.0),
    );
    let _display = scene.add_child_to(container_id, Box::new(key_display));

    let hint = novadraw::RectangleFigure::new_with_color(
        50.0,
        450.0,
        700.0,
        60.0,
...
fn create_scene_6_focus() -> novadraw::FigureGraph {
    let mut scene = novadraw::FigureGraph::new();
    let container = novadraw::RectangleFigure::new(0.0, 0.0, WINDOW_WIDTH, WINDOW_HEIGHT);
    let container_id = scene.set_contents(Box::new(container));

    let focusable1 = novadraw::RectangleFigure::new_with_color(
        150.0,
        100.0,
        200.0,
        80.0,
    ...
    let focusable2 = novadraw::RectangleFigure::new_with_color(
        450.0,
        100.0,
        200.0,
        80.0,
        novadraw::Color::rgba(0.4, 0.6, 0.7, 1.0),
    );
    let focusable3 = novadraw::RectangleFigure::new_with_color(
        150.0,
        250.0,
        200.0,
        80.0,
        novadraw::Color::rgba(0.7, 0.6, 0.4, 1.0),
    );
    let focusable4 = novadraw::RectangleFigure::new_with_color(
        450.0,
        250.0,
        200.0,
        80.0,
        novadraw::Color::rgba(0.5, 0.7, 0.5, 1.0),
    );
...
fn create_scene_7_combo_keys() -> novadraw::FigureGraph {
    let mut scene = novadraw::FigureGraph::new();
    let container = novadraw::RectangleFigure::new(0.0, 0.0, WINDOW_WIDTH, WINDOW_HEIGHT);
    let container_id = scene.set_contents(Box::new(container));

    let combo_area = novadraw::RectangleFigure::new_with_color(
        200.0,
        150.0,
        400.0,
        250.0,
        novadraw::Color::rgba(0.5, 0.5, 0.7, 1.0),
    );
...
fn create_scene_8_event_propagation() -> novadraw::FigureGraph {
    let mut scene = novadraw::FigureGraph::new();
    let container = novadraw::RectangleFigure::new(0.0, 0.0, WINDOW_WIDTH, WINDOW_HEIGHT);
    let container_id = scene.set_contents(Box::new(container));
    ...
    let outer = novadraw::RectangleFigure::new_with_color(
        ...
        600.0,
        400.0,
        novadraw::Color::rgba(0.8, 0.5, 0.5, 1.0),
    );
    let outer_id = scene.add_child_to(container_id, Box::new(outer));

    let middle = novadraw::RectangleFigure::new_with_color(
        150.0,
        130.0,
        400.0,
        250.0,
        novadraw::Color::rgba(0.5, 0.8, 0.5, 1.0),
    );
...
fn create_scene_9_custom_event() -> novadraw::FigureGraph {
    let mut scene = novadraw::FigureGraph::new();
    let container = novadraw::RectangleFigure::new(0.0, 0.0, WINDOW_WIDTH, WINDOW_HEIGHT);
    let container_id = scene.set_contents(Box::new(container));

    let custom_area = novadraw::RectangleFigure::new_with_color(
        250.0,
        200.0,
        300.0,
        150.0,
        novadraw::Color::rgba(0.6, 0.6, 0.4, 1.0),
    );
    let _area = scene.add_child_to(container_id, Box::new(custom_area));

    let log_area = novadraw::RectangleFigure::new_with_color(
        50.0,
        400.0,
        700.0,
        150.0,
